...
export const ShopPurchaseData = {
  get totalSTD() {
    return player.shop.totalSTD;
  },
  get spentSTD() {
    return player.shop.spentSTD;
  },
  set totalSTD(value) {
    player.shop.totalSTD = value;
  },
  set spentSTD(value) {
    player.shop.spentSTD = value;
  },
...
class ShopPurchaseState extends RebuyableMechanicState {
  ...
  get purchases() {
    if (this.config.key === "unlockDLC") return player.hasDLC ? 1 : 0;
    return ShopPurchaseData[this.config.key] ?? 0;
  }

  set purchases(value) {
    if (!Number.isFinite(value)) return;
    if (this.config.key === "unlockDLC") player.hasDLC = Boolean(value);
