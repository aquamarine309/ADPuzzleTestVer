export default (function () { 'use strict';
  ...
  var DEFAULT_FROM_STRING_CACHE_SIZE = (1 << 10) - 1; // The default size of the LRU cache used to cache BE.fromString.
  ...
  var D = function D(value) {
    return BE.fromValue_noAlloc(value);
  };
  var FC = function FC(sign, layer, mag) {
    return BE.fromComponents(sign, layer, mag);
  };
  var FC_NN = function FC_NN(sign, layer, mag) {
    return BE.fromComponents_noNormalize(sign, layer, mag);
  };
  var bePlaces = function bePlaces(value, places) {
    var len = places + 1;
  ...
  function d_lambertw(z) {
    ...
    for (var i = 0; i < 100; ++i) {
      ew = w.neg().exp();
      wewz = w.sub(z.mul(ew));
      wn = w.sub(wewz.div(w.add(1).sub(w.add(2).mul(wewz).div(BE.mul(2, w).add(2)))));
      if (BE.abs(wn.sub(w)).lt(BE.abs(wn).mul(tol))) {
        return wn;
      } else {
        w = wn;
      }
  ...
  /**
   * The value of the BE is sign * 10^10^10...^mag, with (layer) 10s. If the layer is not 0, then negative mag means it's the reciprocal of the corresponding number with positive mag.
   */
  var BE = /*#__PURE__*/function () {
    ...
    _createClass(BE, [{
      ...
      // Object.defineProperty(BE.prototype, "mantissa", {
    }, {
      key: "mantissa",
      get: function get() {
        return this.m;
      ...
      /**
       * Turns the given components into a valid BE.
       */
    }, {
      key: "normalize",
      value:
      /**
       * Turns the BE into a valid BE. This function is meant for internal purposes - users of this library should not need to use normalize.
       *
      ...
      /**
       * Turns the given components into a BE, but not necessarily a valid one (it's only valid if the components would already create a valid BE without normalization). Users of this library should not use this function.
       *
       * Note: this function mutates the BE it is called on.
       */
    }, {
      ...
      /**
       * Turns the mantissa and exponent into a valid BE with value mantissa * 10^exponent.
       *
       * Note: this function mutates the BE it is called on.
       */
    }, {
      ...
      /**
       * Turns the mantissa and exponent into a BE, but not necessarily a valid one. Users of this library should not use this function.
       *
       * Note: this function mutates the BE it is called on.
       */
    }, {
      ...
      value: function fromMantissaExponent_noNormalize(mantissa, exponent) {
        //The idea of 'normalizing' a break_infinity.js style BE doesn't really apply. So just do the same thing.
        this.fromMantissaExponent(mantissa, exponent);
        return this;
      }
      /**
       * Turns the BE that this function is called on into a deep copy of the provided value.
       *
       * Note: this function mutates the BE it is called on.
       */
    }, {
      key: "fromBE",
      value: function fromBE(value) {
      ...
      value: function fromNumber(value) {
        if (Math.abs(value) === Number.POSITIVE_INFINITY) {
          this.mag = 1;
          this.sign = Math.sign(value);
          this.layer = Number.MAX_VALUE;
        } else {
      ...
      value: function fromString(value) {
        ...
        var cached = BE.fromStringCache.get(originalValue);
        if (cached !== undefined) {
          return this.fromBE(cached);
        }
        {
          value = value.replace(",", "");
        }
        ...
        if (pentationparts.length === 2) {
          ...
          if (isFinite(_base) && isFinite(_height)) {
            var result = BE.pentate(_base, _height, payload, linearhyper4);
            this.sign = result.sign;
            this.layer = result.layer;
            this.mag = result.mag;
            if (BE.fromStringCache.maxSize >= 1) {
              BE.fromStringCache.set(originalValue, BE.fromBE(this));
            }
            return this;
          }
        }
        ...
        if (tetrationparts.length === 2) {
          ...
          if (isFinite(_base2) && isFinite(_height2)) {
            var _result = BE.tetrate(_base2, _height2, _payload, linearhyper4);
        ...
        if (powparts.length === 2) {
          ...
          if (isFinite(_base3) && isFinite(_exponent)) {
            var _result2 = BE.pow(_base3, _exponent);
        ...
        if (ptparts.length === 2) {
          ...
          if (isFinite(base) && isFinite(height)) {
            var _result3 = BE.tetrate(base, height, _payload2, linearhyper4);
        ...
        if (ptparts.length === 2) {
          ...
          if (isFinite(base) && isFinite(height)) {
            var _result4 = BE.tetrate(base, height, _payload3, linearhyper4);
        ...
        if (ptparts.length === 2) {
          ...
          if (isFinite(base) && isFinite(height)) {
            var _result5 = BE.tetrate(base, height, _payload4, linearhyper4);
        ...
        //Handle numbers written like Xeee... (N es) Y
          ...
          this.layer = ecount;
          if (ecount === 2) {
            var _result6 = BE.mul(FC(1, 2, exponent), D(mantissa));
      ...
      /**
       * The function used by new BE() to create a new BE. Accepts a BESource: uses fromNumber if given a number, uses fromString if given a string, and uses fromBE if given a BE.
       *
       * Note: this function mutates the BE it is called on.
       */
    }, {
      ...
      /**
       * Returns the numeric value of the BE it's called on. Will return Infinity (or -Infinity for negatives) for BEs that are larger than Number.MAX_VALUE.
       */
    }, {
      key: "toNumber",
      value: function toNumber() {
        ...
        } //overflow for any normalized BE
        else {
      ...
      key: "mantissaWithBEPlaces",
      value: function mantissaWithBEPlaces(places) {
        ...
        return bePlaces(this.m, places);
      }
    }, {
      key: "magnitudeWithBEPlaces",
      value: function magnitudeWithBEPlaces(places) {
        // https://stackoverflow.com/a/37425022
        if (isNaN(this.mag)) {
          return Number.NaN;
        }
        ...
        return bePlaces(this.mag, places);
      }
      /**
       * Returns a string representation of the BE it's called on.
      ...
      value: function toExponential(places) {
        ...
        return this.toStringWithBEPlaces(places);
      }
    }, {
      key: "toFixed",
      ...
      key: "toStringWithBEPlaces",
      value: function toStringWithBEPlaces(places) {
        if (this.layer === 0) {
          if (this.mag < 1e21 && this.mag > 1e-7 || this.mag === 0) {
            return (this.sign * this.mag).toFixed(places);
          }
          return bePlaces(this.m, places) + "e" + bePlaces(this.e, places);
        } else if (this.layer === 1) {
          return bePlaces(this.m, places) + "e" + bePlaces(this.e, places);
        } else {
          //layer 2+
          if (this.layer <= MAX_ES_IN_A_ROW) {
            return (this.sign === -1 ? "-" : "") + "e".repeat(this.layer) + bePlaces(this.mag, places);
          } else {
            return (this.sign === -1 ? "-" : "") + "(e^" + this.layer + ")" + bePlaces(this.mag, places);
          }
        }
      }
      /**
      ...
      /**
       * Negates the BE it's called on: in other words, when given X, returns -X.
       */
    }, {
      key: "neg",
      value: function neg() {
      ...
      //   }
      /**
       * Returns the sign of the BE it's called on. (Though, since sign is a public data member of BE, you might as well just call .sign instead of .sgn())
       */
    }, {
      key: "sgn",
      value: function sgn() {
      ...
      /**
       * Rounds the BE it's called on to the nearest integer.
       */
    }, {
      key: "round",
      ...
      /**
       * "Rounds" the BE it's called on to the nearest integer that's less than or equal to it.
       */
    }, {
      key: "floor",
      ...
      /**
       * "Rounds" the BE it's called on to the nearest integer that's greater than or equal to it.
       */
    }, {
      key: "ceil",
      value: function ceil() {
      ...
      /**
       * Extracts the integer part of the BE and returns it. Behaves like floor on positive numbers, but behaves like ceiling on negative numbers.
       */
    }, {
      key: "trunc",
      ...
      value: function add(value) {
        ...
        if (this.eq(BE.dInf) && be.eq(BE.dNegInf) || this.eq(BE.dNegInf) && be.eq(BE.dInf)) {
        ...
        if (this.sign === -be.sign && this.layer === be.layer && this.mag === be.mag) {
          return FC_NN(0, 0, 0);
        }
        var a;
        var b;
        //Special case: If one of the numbers is layer 2 or higher, just take the bigger number.
        if (this.layer >= 2 || be.layer >= 2) {
          return this.maxabs(be);
        }
        if (BE.cmpabs(this, be) > 0) {
          a = new BE(this);
          b = new BE(be);
        } else {
          a = new BE(be);
        ...
        if (a.layer === 0 && b.layer === 0) {
          return BE.fromNumber(a.sign * a.mag + b.sign * b.mag);
        }
      ...
      value: function mul(value) {
        ...
        //Infinity * 0 = NaN
        if (this.mag == Number.POSITIVE_INFINITY && be.eq(BE.dZero) || this.eq(BE.dZero) && this.mag == Number.POSITIVE_INFINITY) {
          return FC_NN(Number.NaN, Number.NaN, Number.NaN);
        }
        //inf/nan check
        if (!Number.isFinite(this.layer)) {
        ...
        if (this.sign === 0 || be.sign === 0) {
          return FC_NN(0, 0, 0);
        }
        //Special case - Multiplying a number by its own reciprocal yields +/- 1, no matter how large.
        if (this.layer === be.layer && this.mag === -be.mag) {
          return FC_NN(this.sign * be.sign, 0, 1);
        }
        var a;
        var b;
        //Which number is bigger in terms of its multiplicative distance from 1?
        if (this.layer > be.layer || this.layer == be.layer && Math.abs(this.mag) > Math.abs(be.mag)) {
          a = new BE(this);
          b = new BE(be);
        } else {
          a = new BE(be);
        ...
        if (a.layer === 0 && b.layer === 0) {
          return BE.fromNumber(a.sign * b.sign * a.mag * b.mag);
        }
      ...
      /**
       * Returns the reciprocal (1 / X) of the BE it's called on.
       */
    }, {
      key: "recip",
      ...
      value: function mod(value) {
        var be = D(value).abs();
        if (be.eq(BE.dZero)) return FC_NN(0, 0, 0);
        var num_this = this.toNumber();
        var num_be = be.toNumber();
        //Special case: To avoid precision issues, if both numbers are valid JS numbers, just call % on those
        if (isFinite(num_this) && isFinite(num_be) && num_this != 0 && num_be != 0) {
          return new BE(num_this % num_be);
        }
        if (this.sub(be).eq(this)) {
          //be is too small to register to this
          return FC_NN(0, 0, 0);
        }
        if (be.sub(this).eq(be)) {
          //this is too small to register to be
          return new BE(this);
        }
        if (this.sign == -1) return this.abs().mod(be).neg();
        return this.sub(this.div(be).floor().mul(be));
      }
      /**
       * Returns the remainder of 'this' divided by 'value': for example, 5 mod 2 = 1, because the remainder of 5 / 2 is 1.
       * Uses the "truncated division" modulo, which is the same as JavaScript's native modulo operator (%).
      ...
      value: function cmpabs(value) {
        ...
        var layerb = be.mag > 0 ? be.layer : -be.layer;
        if (layera > layerb) {
          return 1;
        }
        if (layera < layerb) {
      ...
      /**
       * Returns true if the BE is an NaN value.
       */
    }, {
      key: "isNan",
      ...
      /**
       * Returns true if the BE is finite (by BE standards, not by floating point standards - a humongous BE like 10^^10^100 is still finite!)
       */
    }, {
      key: "isFinite",
      ...
      /**
       * The BE equivalent of ==. Returns true if 'this' and 'value' have equal values.
       */)
    }, {
      key: "eq",
      value: function eq(value) {
        var be = D(value);
        return this.sign === be.sign && this.layer === be.layer && this.mag === be.mag;
      }
      /**
      ...
      /**
       * The BE equivalent of !=. Returns true if 'this' and 'value' do not have equal values.
       */
    }, {
      key: "neq",
      ...
      /**
       * The BE equivalent of <. Returns true if 'this' is less than 'value'.
       */
    }, {
      key: "lt",
      ...
      /**
       * The BE equivalent of <=. Returns true if 'this' is less than or equal to 'value'.
       */
    }, {
      key: "lte",
      ...
      /**
       * The BE equivalent of >. Returns true if 'this' is greater than 'value'.
       */
    }, {
      key: "gt",
      ...
      /**
       * The BE equivalent of >=. Returns true if 'this' is greater than or equal to 'value'.
       */
    }, {
      key: "gte",
      ...
      value: function max(value) {
        var be = D(value);
        return this.lt(be) ? new BE(be) : new BE(this);
      }
      /**
      ...
      value: function min(value) {
        var be = D(value);
        return this.gt(be) ? new BE(be) : new BE(this);
      }
      /**
      ...
      value: function maxabs(value) {
        var be = D(value);
        return this.cmpabs(be) < 0 ? new BE(be) : new BE(this);
      }
      /**
       ...
       * However, the two BEs are considered equal if they're approximately equal up to a certain tolerance.
      ...
      value: function cmp_tolerance(value, tolerance) {
        var be = D(value);
        return this.eq_tolerance(be, tolerance) ? 0 : this.cmp(be);
      }
      /**
       * Returns 1 if 'this' is greater than 'value', returns -1 if 'this' is less than 'value', returns 0 if 'this' is equal to 'value'.
       * However, the two BEs are considered equal if they're approximately equal up to a certain tolerance.
      ...
      /**
       * Tests whether two BEs are approximately equal, up to a certain tolerance.
       * Tolerance is a relative tolerance, multiplied by the greater of the magnitudes of the two arguments.
       * For example, if you put in 1e-9, then any number closer to the
       * larger number than (larger number)*1e-9 will be considered equal.
       */
      ...
      value: function eq_tolerance(value, tolerance) {
        var be = D(value); // https://stackoverflow.com/a/33024979
        if (tolerance == null) {
          tolerance = 1e-7;
        }
        ...
        if (Math.abs(this.layer - be.layer) > 1) {
          return false;
        }
        // return abs(a-b) <= tolerance * max(abs(a), abs(b))
        var magA = this.mag;
        var magB = be.mag;
        if (this.layer > be.layer) {
      ...
      /**
       * Tests whether two BEs are not approximately equal, up to a certain tolerance.
       * Tolerance is a relative tolerance, multiplied by the greater of the magnitudes of the two arguments.
       * For example, if you put in 1e-9, then any number closer to the
       * larger number than (larger number)*1e-9 will be considered equal.
       */
      ...
      value: function lt_tolerance(value, tolerance) {
        var be = D(value);
        return !this.eq_tolerance(be, tolerance) && this.lt(be);
      }
      /**
      ...
      value: function gt_tolerance(value, tolerance) {
        var be = D(value);
        return !this.eq_tolerance(be, tolerance) && this.gt(be);
      }
      /**
      ...
      /**
       * "Positive log10": Returns the base-10 logarithm of nonnegative BEs, but returns 0 for negative BEs.
       */
    }, {
      key: "pLog10",
      value: function pLog10() {
        if (this.lte(BE.dZero)) {
          return FC_NN(0, 0, 0);
        }
        return this.log10();
      }
      ...
      /**
       * Base-10 logarithm: returns the BE X such that 10^X = 'this'.
      ...
      /**
       * Logarithms are one of the inverses of exponentiation: this function finds the BE X such that base^X = 'this'.
       */
    }, {
      key: "log",
      value: function log(base) {
        ...
        return BE.div(this.log10(), base.log10());
      }
      /**
       * Base-2 logarithm: returns the BE X such that 2^X = 'this'.
       */
    }, {
      key: "log2",
      value: function log2() {
      ...
      /**
       * Base-e logarithm, also known as the "natural" logarithm: returns the BE X such that e^X = 'this'.
       */
    }, {
      key: "ln",
      value: function ln() {
      ...
      /**
       * Roots are one of the inverses of exponentiation: this function finds the BE X such that X ^ 'value' = 'this'.
      ...
      value: function factorial() {
        if (this.mag < 0) {
          ...
          return BE.exp(BE.mul(this, BE.ln(this).sub(1)));
        } else {
          return BE.exp(this);
        }
      }
      ...
      value: function gamma() {
        if (this.mag < 0) {
          ...
          if (this.lt(FC_NN(1, 0, 24))) {
            return BE.fromNumber(f_gamma(this.sign * this.mag));
          }
          var t = this.mag - 1;
          var l = 0.9189385332046727; //0.5*Math.log(2*Math.PI)
      ...
      value: function exp() {
        ...
        if (this.layer === 0 && this.mag <= 709.7) {
          return BE.fromNumber(Math.exp(this.sign * this.mag));
        } else if (this.layer === 0) {
      ...
      /**
       * Square root: finds the BE X such that X * X, a.k.a X^2, equals 'this'. Equivalent to X^(1/2).
       */
    }, {
      key: "sqrt",
      value: function sqrt() {
        if (this.layer === 0) {
          return BE.fromNumber(Math.sqrt(this.sign * this.mag));
        } else if (this.layer === 1) {
          return FC(1, 2, Math.log10(this.mag) - 0.3010299956639812);
        } else {
          var result = BE.div(FC_NN(this.sign, this.layer - 1, this.mag), FC_NN(1, 0, 2));
          result.layer += 1;
          result.normalize();
          return result;
        }
      ...
      /**
       * Cube root: finds the BE X such that X^3 equals 'this'. Equivalent to X^(1/3).
       */
    }, {
      key: "cbrt",
      value: function cbrt() {
      ...
      value: function tetrate() {
        ...
        //x^^1 == x
        if (height === 1) {
          return BE.pow(this, payload);
        }
        //x^^0 == 1
        if (height === 0) {
          return new BE(payload);
        }
        //1^^x == 1
        if (this.eq(BE.dOne)) {
          return FC_NN(1, 0, 1);
        ...
        if (height === Number.POSITIVE_INFINITY) {
          ...
          if (this_num <= 1.44466786100976613366 && this_num >= 0.06598803584531253708) {
            ...
            if (this_num > 1.444667861009099) {
              lower = upper = BE.fromNumber(Math.E);
            }
            payload = D(payload);
        ...
        if (this.eq(BE.dZero)) {
          ...
          return BE.fromNumber(result);
        }
        if (height < 0) {
          return BE.iteratedlog(payload, this, -height, linear);
        }
        payload = new BE(payload);
        var oldheight = height;
        height = Math.trunc(height);
        var fracheight = oldheight - height;
        if (this.gt(BE.dZero) && (this.lt(1) || this.lte(1.44466786100976613366) && payload.lte(BE.ln(this).neg().lambertw(false).div(BE.ln(this).neg()))) && (oldheight > 10000 || !linear)) {
          ...
          if (payload.eq(BE.dOne)) payload = this.pow(fracheight);else if (this.lt(1)) payload = payload.pow(1 - fracheight).mul(this.pow(payload).pow(fracheight));else payload = payload.layeradd(fracheight, this);
        ...
        if (fracheight !== 0) {
          if (payload.eq(BE.dOne)) {
            ...
            if (this.gt(10) || linear) {
              ...
              payload = BE.fromNumber(BE.tetrate_critical(this.toNumber(), fracheight));
      ...
      value: function iteratedlog() {
        ...
        var linear = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
        if (times < 0) {
          return BE.tetrate(base, -times, this, linear);
        }
        base = D(base);
        var result = BE.fromBE(this);
        var fulltimes = times;
      ...
      value: function slog() {
        ...
        for (var i = 1; i < iterations; ++i) {
          var new_be = new BE(base).tetrate(result, BE.dOne, linear);
          var currently_rose = new_be.gt(this);
          if (i > 1) {
      ...
      value: function slog_internal() {
        ...
        //slog_n(0) is -1
        if (this.mag < 0 || this.eq(BE.dZero)) {
          return FC_NN(-1, 0, 1);
        }
        if (base.lt(1.44466786100976613366)) {
          var negln = BE.ln(base).neg();
        ...
        for (var i = 0; i < 100; ++i) {
          if (copy.lt(BE.dZero)) {
            copy = BE.pow(base, copy);
            result -= 1;
          } else if (copy.lte(BE.dOne)) {
            if (linear) return BE.fromNumber(result + copy.toNumber() - 1);else return BE.fromNumber(result + BE.slog_critical(base.toNumber(), copy.toNumber()));
          } else {
            result += 1;
            copy = BE.log(copy, base);
          }
        }
        return BE.fromNumber(result);
      }
      ...
      value:
      /**
       * Adds/removes layers from a BE, even fractional layers (e.g. its slog10 representation). Very similar to tetrate base 10 and iterated log base 10.
       *
       * Tetration for non-integer heights does not have a single agreed-upon definition,
       * so this library uses an analytic approximation for bases <= 10, but it reverts to the linear approximation for bases > 10.
       * If you want to use the linear approximation even for bases <= 10, set the linear parameter to true.
      ...
      function layeradd10(diff) {
        var linear = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
        diff = BE.fromValue_noAlloc(diff).toNumber();
        var result = BE.fromBE(this);
        if (diff >= 1) {
      ...
      value: function layeradd(diff, base) {
        ...
        if (baseD.gt(1) && baseD.lte(1.44466786100976613366)) {
          var excessSlog = BE.excess_slog(this, base, linear);
          ...
          return BE.tetrate(baseD, wholeheight, towertop, linear); //wholediff is a whole number so this is safe even if it ends up calling iteratedlog
        }

        var slogthis = this.slog(base, 100, linear).toNumber();
        var slogdest = slogthis + diff;
        if (slogdest >= 0) {
          return BE.tetrate(base, slogdest, BE.dOne, linear);
        } else if (!Number.isFinite(slogdest)) {
          return FC_NN(Number.NaN, Number.NaN, Number.NaN);
        } else if (slogdest >= -1) {
          return BE.log(BE.tetrate(base, slogdest + 1, BE.dOne, linear), base);
        } else {
          return BE.log(BE.log(BE.tetrate(base, slogdest + 2, BE.dOne, linear), base), base);
        }
      }
       ...
       * Returns a pair of a BE and a number, with the number always being 0, 1, or 2. The number indicates what range we're in:
      ...
      function lambertw() {
        ...
        if (this.lt(-0.3678794411710499)) {
          ...
          if (this.abs().lt("1e-300")) return new BE(this);else if (this.mag < 0) {
            return BE.fromNumber(f_lambertw(this.toNumber()));
          } else if (this.layer === 0) {
            return BE.fromNumber(f_lambertw(this.sign * this.mag));
          ...
          if (this.layer === 0) {
            return BE.fromNumber(f_lambertw(this.sign * this.mag, 1e-10, false));
          } else if (this.layer == 1) {
      ...
      value: function linear_sroot(degree) {
        ...
        if (degree > -2 && degree < -1) {
          return BE.fromNumber(degree).add(2).pow(this.recip());
        }
        ...
        if (this.lte("1ee-16")) {
          if (degree % 2 == 1) return new BE(this);else return FC_NN(Number.NaN, Number.NaN, Number.NaN);
        }
        //this > 1
        if (this.gt(1)) {
          ...
          var upperBound = BE.dTen;
          if (this.gte(BE.tetrate(10, degree, 1, true))) {
            upperBound = this.iteratedlog(10, degree - 1, true);
          }
          if (degree <= 1) {
            upperBound = this.root(degree);
          }
          var lower = BE.dZero; //This is zero rather than one because we might be on a higher layer, so the lower bound might actually some 10^10^10...^0
          ...
          while (loopGoing) {
            ...
            if (BE.iteratedexp(10, layer, guess, true).tetrate(degree, 1, true).gt(this)) upper = guess;else lower = guess;
            if (guess.eq(previous)) loopGoing = false;else previous = guess;
          }
          return BE.iteratedexp(10, layer, guess, true);
        }
        //0 < this < 1
        else {
          //A tetration of be degree can potentially have three different portions, as shown at https://www.desmos.com/calculator/ayvqks6mxa, which is a graph of x^^2.05:
          ...
          var _lower = FC(1, 10, 1); //eeeeeeeee-10, which is effectively 0; I would use BE.dInf but its reciprocal is NaN
          var _upper = FC(1, 1, -16); //~ 1 - 1e-16
          var prevspan = BE.dZero;
          var difference = FC(1, 10, 1);
          var _upperBound = _upper.pow10().recip();
          var distance = BE.dZero;
          ...
          while (_loopGoing) {
            ...
            if (BE.pow(10, _guess).recip().tetrate(degree, 1, true).gt(this)) _upper = _guess;else _lower = _guess;
      ...
      /**
       * Pentation/pentate: The result of tetrating 'height' times in a row. An absurdly strong operator - BE.pentate(2, 4.28) and BE.pentate(10, 2.37) are already too huge for break_eternity.js!
      ...
      value: function pentate() {
        ...
        if (fracheight !== 0) {
          if (payload.eq(BE.dOne)) {
            ++height;
            payload = BE.fromNumber(fracheight);
          } else {
            if (this.eq(10)) {
              payload = payload.layeradd10(fracheight, linear);
            } else {
        ...
        for (var i = 0; i < height; ++i) {
          payload = this.tetrate(payload.toNumber(), BE.dOne, linear);
          //bail if we're NaN
          if (!isFinite(payload.layer) || !isFinite(payload.mag)) {
            return payload.normalize();
          }
      ...
      value: function sin() {
        ...
        if (this.layer === 0) {
          return BE.fromNumber(Math.sin(this.sign * this.mag));
        }
        return FC_NN(0, 0, 0);
      }
      /**
      ...
      value: function cos() {
        ...
        if (this.layer === 0) {
          return BE.fromNumber(Math.cos(this.sign * this.mag));
        }
        return FC_NN(0, 0, 0);
      }
      /**
      ...
      value: function tan() {
        ...
        if (this.layer === 0) {
          return BE.fromNumber(Math.tan(this.sign * this.mag));
        }
        return FC_NN(0, 0, 0);
      }
      /**
      ...
      value: function asin() {
        ...
        if (this.layer === 0) {
          return BE.fromNumber(Math.asin(this.sign * this.mag));
        }
        return FC_NN(Number.NaN, Number.NaN, Number.NaN);
      }
      /**
      ...
      value: function acos() {
        if (this.mag < 0) {
          return BE.fromNumber(Math.acos(this.toNumber()));
        }
        if (this.layer === 0) {
          return BE.fromNumber(Math.acos(this.sign * this.mag));
        }
        return FC_NN(Number.NaN, Number.NaN, Number.NaN);
      }
      /**
      ...
      value: function atan() {
        ...
        if (this.layer === 0) {
          return BE.fromNumber(Math.atan(this.sign * this.mag));
        }
        return BE.fromNumber(Math.atan(this.sign * 1.8e308));
      }
      /**
      ...
      value: function asinh() {
        return BE.ln(this.add(this.sqr().add(1).sqrt()));
      }
      /**
      ...
      value: function acosh() {
        return BE.ln(this.add(this.sqr().sub(1).sqrt()));
      }
      /**
      ...
      value: function atanh() {
        ...
        return BE.ln(this.add(1).div(BE.fromNumber(1).sub(this))).div(2);
      }
      /**
      ...
      value: function ascensionPenalty(ascensions) {
        ...
        return this.root(BE.pow(10, ascensions));
      }
      /**
      ...
      key: "fromComponents",
      value: function fromComponents(sign, layer, mag) {
        return new BE().fromComponents(sign, layer, mag);
      }
      /**
       * Turns the given components into a BE, but not necessarily a valid one (it's only valid if the components would already create a valid BE without normalization). Users of this library should not use this function.
       */
      ...
      key: "fromComponents_noNormalize",
      value: function fromComponents_noNormalize(sign, layer, mag) {
        return new BE().fromComponents_noNormalize(sign, layer, mag);
      }
      /**
       * Turns the mantissa and exponent into a valid BE with value mantissa * 10^exponent.
       */
      ...
      key: "fromMantissaExponent",
      value: function fromMantissaExponent(mantissa, exponent) {
        return new BE().fromMantissaExponent(mantissa, exponent);
      }
      /**
       * Turns the mantissa and exponent into a BE, but not necessarily a valid one. Users of this library should not use this function.
       */
      ...
      key: "fromMantissaExponent_noNormalize",
      value: function fromMantissaExponent_noNormalize(mantissa, exponent) {
        return new BE().fromMantissaExponent_noNormalize(mantissa, exponent);
      }
      /**
      ...
      value: function fromString(value) {
        var linearhyper4 = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
        return new BE().fromString(value, linearhyper4);
      }
      /**
       * The function used by new BE() to create a new BE. Accepts a BESource: uses fromNumber if given a number, uses fromString if given a string, and uses fromBE if given a BE.
       */
      ...
      key: "fromValue",
      value: function fromValue(value) {
        return new BE().fromValue(value);
      }
      /**
       * Converts a BESource to a BE, without constructing a new BE
       * if the provided value is already a BE.
       *
       * As the return value could be the provided value itself, this function
       * returns a read-only BE to prevent accidental mutations of the value.
       * Use `new BE(value)` to explicitly create a writeable copy if mutation
       * is required.
       */
    }, {
      ...
      value: function fromValue_noAlloc(value) {
        if (value instanceof BE) {
          ...
          return BE.fromString(value);
        } else if (typeof value === "number") {
          return BE.fromNumber(value);
        } else {
          // This should never happen... but some users like Prestige Tree Rewritten
          // pass undefined values in as BESources, so we should handle this
      ...
      /**
       * Addition: returns the sum of the two BEs.
       */
    }, {
      key: "add",
      ...
      /**
       * Multiplication: returns the product of the two BEs.
       */
    }, {
      key: "mul",
      ...
      /**
       * Returns true if the given value is finite (by BE standards, not by floating point standards - a humongous BE like 10^^10^100 is still finite!)
       */)
    }, {
      key: "isFinite",
      ...
      /**
       * The BE equivalent of ==. Returns true if 'value' and 'other' have equal values.
       */)
    }, {
      key: "eq",
      ...
      /**
       * The BE equivalent of !=. Returns true if 'value' and 'other' do not have equal values.
       */
    }, {
      key: "neq",
      ...
      /**
       * The BE equivalent of <. Returns true if 'value' is less than 'other'.
       */
    }, {
      key: "lt",
      ...
      /**
       * The BE equivalent of <=. Returns true if 'value' is less than or equal to 'other'.
       */
    }, {
      key: "lte",
      ...
      /**
       * The BE equivalent of >. Returns true if 'value' is greater than 'other'.
       */
    }, {
      key: "gt",
      ...
      /**
       * The BE equivalent of >=. Returns true if 'value' is greater than or equal to 'other'.
       */
    }, {
      key: "gte",
      ...
      /**
       * Base-10 logarithm: returns the BE X such that 10^X = 'value'.
       * For numbers above layer 0, this is equivalent to subtracting 1 from layer and normalizing.
       */
    }, {
      key: "log10",
      ...
      /**
       * Logarithms are one of the inverses of exponentiation: this function finds the BE X such that base^X = 'value'.
       */
    }, {
      key: "log",
      ...
      /**
       * Base-2 logarithm: returns the BE X such that 2^X = 'value'.
       */
    }, {
      key: "log2",
      ...
      /**
       * Base-e logarithm, also known as the "natural" logarithm: returns the BE X such that e^X = 'value'.
       */
    }, {
      key: "ln",
      ...
      /**
       * Roots are one of the inverses of exponentiation: this function finds the BE X such that X ^ 'other' = 'value'.
      ...
      /**
       * Square root: finds the BE X such that X * X, a.k.a X^2, equals 'value'. Equivalent to X^(1/2).
       */
    }, {
      key: "sqrt",
      ...
      /**
       * Cube root: finds the BE X such that X^3 equals 'value'. Equivalent to X^(1/3).
       */
    }, {
      key: "cbrt",
      ...
      key: "randomBEForTesting",
      value: function randomBEForTesting(maxLayers) {
      ...
      value: function affordGeometricSeries_core(resourcesAvailable, priceStart, priceRatio, currentOwned) {
        ...
        return BE.floor(resourcesAvailable.div(actualStart).mul(priceRatio.sub(1)).add(1).log10().div(priceRatio.log10()));
      }
    }, {
      key: "sumGeometricSeries_core",
      value: function sumGeometricSeries_core(numItems, priceStart, priceRatio, currentOwned) {
        return priceStart.mul(priceRatio.pow(currentOwned)).mul(BE.sub(1, priceRatio.pow(numItems))).div(BE.sub(1, priceRatio));
      }
    }, {
      ...
      value: function slog_critical(base, height) {
        ...
        return BE.critical_section(base, height, critical_slog_values);
      }
    }, {
      key: "tetrate_critical",
      value: function tetrate_critical(base, height) {
        return BE.critical_section(base, height, critical_tetr_values);
      }
    }, {
      ...
      value: function excess_slog(value, base) {
        ...
        if (value.gt(upper)) {
          ...
          while (step_size > 1e-16) {
            ...
            guess = BE.iteratedexp(base, estimate, towertop);
            if (guess.eq(value)) return [new BE(estimate + tested), 2];else if (guess.lt(value)) fracheight += step_size;
            step_size /= 2;
          }
          if (guess.neq_tolerance(value, 1e-7)) return [FC_NN(Number.NaN, Number.NaN, Number.NaN), 0];
          return [new BE(estimate + fracheight), 2];
        }
        if (value.lt(upper) && value.gt(lower)) {
          ...
          while (_step_size > 1e-16) {
            ...
            _guess2 = BE.iteratedexp(base, _estimate, _towertop);
            if (_guess2.eq(value)) return [new BE(_estimate + _tested), 1];else if (_guess2.gt(value)) _fracheight += _step_size;
            _step_size /= 2;
          }
          if (_guess2.neq_tolerance(value, 1e-7)) return [FC_NN(Number.NaN, Number.NaN, Number.NaN), 0];
          return [new BE(_estimate + _fracheight), 1];
        }
  ...
  BE.dZero = FC_NN(0, 0, 0);
  BE.dOne = FC_NN(1, 0, 1);
  BE.dNegOne = FC_NN(-1, 0, 1);
  BE.dTwo = FC_NN(1, 0, 2);
  BE.dTen = FC_NN(1, 0, 10);
  BE.dNaN = FC_NN(Number.NaN, Number.NaN, Number.NaN);
  BE.dInf = FC_NN(1, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY);
  BE.dNegInf = FC_NN(-1, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY);
  ...
  BE.MAX_VALUE = FC_NN(1, EXP_LIMIT, 1);
  // return BE;
  // Optimise BE aliases.
  // We can't do this optimisation before BE is assigned.
  D = BE.fromValue_noAlloc;
  FC = BE.fromComponents;
  FC_NN = BE.fromComponents_noNormalize;
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  BE.fromMantissaExponent;
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  BE.fromMantissaExponent_noNormalize;
